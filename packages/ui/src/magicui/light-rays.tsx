...
const Ray = ({ left, rotate, width, swing, delay, duration, intensity, fromBottom }: LightRay & { fromBottom?: boolean }) => {
  return (
    <motion.div
      ...
      initial={{ rotate: fromBottom ? -rotate : rotate }}
      animate={{
        opacity: [0, intensity, 0],
        rotate: fromBottom ? [-rotate - swing, -rotate + swing, -rotate - swing] : [rotate - swing, rotate + swing, rotate - swing]
      }}
      transition={{
        duration: duration,
        repeat: Infinity,
...
export function LightRays({ className, style, count = 7, color = 'rgba(160, 210, 255, 0.2)', blur = 36, speed = 14, length = '70vh', fromBottom = false, ref, ...props }: LightRaysProps) {
