...
export const Attachment = ({ data, onRemove, className, children, ...props }: AttachmentProps) => {
  ...
  return (
    <AttachmentContext.Provider value={contextValue}>
      <div
        className={cn('group relative', variant === 'grid' && 'size-24 overflow-hidden rounded-lg', variant === 'inline' && ['flex h-8 cursor-pointer items-center gap-1.5 select-none', 'rounded-md border border-border px-1.5', 'text-sm font-medium transition-all', 'hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50'], variant === 'list' && ['flex w-full items-center gap-3 rounded-lg border p-3', 'hover:bg-accent/50'], className)}
        {...props}
      >
        {children}
      </div>
...
export const AttachmentRemove = ({ label = 'Remove', className, children, ...props }: AttachmentRemoveProps) => {
  ...
  return (
    <Button
      aria-label={label}
      className={cn(variant === 'grid' && ['absolute top-2 right-2 size-6 rounded-full p-0', 'bg-background/80 backdrop-blur-sm', 'opacity-0 transition-opacity group-hover:opacity-100', 'hover:bg-background', '[&>svg]:size-3'], variant === 'inline' && ['size-5 rounded p-0', 'opacity-0 transition-opacity group-hover:opacity-100', '[&>svg]:size-2.5'], variant === 'list' && ['size-8 shrink-0 rounded p-0', '[&>svg]:size-4'], className)}
      onClick={handleClick}
      type="button"
      variant="ghost"
      {...props}
