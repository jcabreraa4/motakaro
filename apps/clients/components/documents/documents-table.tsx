...
function DocumentsRow({ document }: { document: Document }) {
  ...
  const updateDocument = useMutation(api.documents.clientUpdate);
  ...
  function handleUpdate() {
    updateDocument({ id: document._id, clientStarred: !document.clientStarred })
      .then(() => toast.success(document.clientStarred ? 'Document unstarred successfully.' : 'Document starred successfully.'))
  ...
  return (
    <TableRow className="h-12 cursor-pointer p-20">
      ...
      >
        {document.clientStarred ? <StarIcon className="text-yellow-500" /> : <FileTextIcon />}
      </TableCell>
      <TableCell
        onClick={openInternally}
      ...
      <TableCell className="text-right">
        <DropdownMenu>
          ...
          >
            ...
            >
              {document.clientStarred ? <StarOffIcon /> : <StarIcon />}
              {document.clientStarred ? 'Quit from Favorites' : 'Add to Favorites'}
            </DropdownMenuItem>
            <DropdownMenuItem
...
export function DocumentsTable({ documents }: { documents: Document[] }) {
  const starredDocuments = documents.filter((document) => document.clientStarred);
  const nonStarredDocuments = documents.filter((document) => !document.clientStarred);
